...
class v2_minimax_tts(TTS):
    def __init__(self, config:secret):
        self.group_id = config.get("minimax_v2", "group_id")
        self.api_key = config.get("minimax_v2", "key")
    ...
    def generate(self, text, voiceId, speed:float=1.0):
        ...
        if "data" in parsed_json:
            ...
            print("调用失败", voiceId, response.status_code, response.text)
            return None
        return audio_value
    ...
    def generate_mix(self, text, voiceIds, weights, speed:float=1.0):
        ...
        if "data" in parsed_json:
            ...
            print("调用失败", voiceIds, response.status_code, response.text)
            return None
        return audio_value
